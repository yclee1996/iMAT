...
def perodic_table(molecule):
    ...
    metal_mass = [44.956,47.880,50.941,51.996,54.938,55.847,58.933,58.690,0,0]
    metal_zval = [11,10,11,12,13,8,9,10,0,0]
    halo_atom = ['F','Cl','Br','I']
    halo_electron = [9,17,35,53]
    halo_en = [3.98,3.16,2.96,2.66]
    halo_mass = [18.998,35.453,79.904,126.904]
    for i in metal_atom:
        if i in molecule[-31:-26]:
            ...
            return_list.append(metal_mass[c])
    for j in halo_atom:
        if j in molecule[-28:-25]:
            ...
            return_list.append(halo_mass[c])
    return return_list
######################################
...
def get_coord(the_list):
    ...
    for c in the_list:
        try:
            ...
            pass
    a = len(energy_list)
    parameter = np.array(para_list).reshape(a,12)
...
for alpha in alpha_list:
    for unit in unit_list:
        ...
        with open("NN_result_3layer_0.6", "a") as p:
            p.write('mass_minus_electron the error of ' + str(unit) + ' unit in each of 3 layers with alpha = '+ str(alpha) +' is '+ 'train: ' + str(train) + ' test: ' + str(test)+"\n")
