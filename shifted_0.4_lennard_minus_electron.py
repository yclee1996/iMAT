...
def get_coord(the_list):
    ...
    for c in the_list:
        try:
            with open(c, "r") as p:
                for i, line in enumerate(p):
                    ...
                    if "#" not in line:
                        ...
                        para_list.append((1/length)**12-(1/length)**6)
            ...
            pass
    a = len(energy_list)
    parameter = np.array(para_list).reshape(a,11)
...
for alpha in alpha_list:
    for unit in unit_list:
        ...
        with open("NN_result_3layer_0.4", "a") as p:
            p.write('lennard_minus_electron the error of ' + str(unit) + ' unit in each of 3 layers with alpha = '+ str(alpha) +' is '+ 'train: ' + str(train) + ' test: ' + str(test)+"\n")
