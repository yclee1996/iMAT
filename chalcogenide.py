...
def perodic_table(x):
    return_list = []
    metal_atom = ['Sc','Ti','V','Cr','Mn','Fe','Co','Ni','Cu','Zn','Nb']
    chalcogenide_atom = ['O','ZZZZ','ZZZZZ','Te']
    for i in metal_atom:
        if i in x[-31:-26]:
            return_list.append(metal_atom.index(i))
    for j in chalcogenide_atom:
        if j in x[-28:-25]:
            return_list.append(chalcogenide_atom.index(j))
    if ('S' in x[-28:-25]) and ('Se' not in x[-28:-25]):
...
def get_all_file(directory):
    ...
    for a in dire:
        ...
        for i in os.listdir():
            return_list.append(os.getcwd()+"/"+i+"/"+"vector_vs_energy_add_geo")
   #     num_list = []
   #     for y in os.listdir():
   #         try:
   #             x = float(y)
        ...
        try:
            with open(c, "r") as p:
                for i, line in enumerate(p):
                    ...
                    if "#" not in line:
                        try:
                            a = str_list2float_list(line.split())
...
X_train, X_test, y_train, y_test = train_test_split(scaled_para, y, random_state = 3)
######################################
...
ax1.scatter(X_test[:50,1],regress(500,0.0001)[:50], s=10, c='g', marker="o", label='NN Prediction')
