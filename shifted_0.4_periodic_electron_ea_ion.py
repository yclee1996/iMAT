...
def get_coord(the_list):
    ...
    for c in the_list:
        try:
            ...
            pass
    a = len(energy_list)
    parameter = np.array(para_list).reshape(a,16)
...
for alpha in alpha_list:
    for unit in unit_list:
        ...
        with open("NN_result_3layer_0.4", "a") as p:
            p.write('ea_ion_periodic_electron the error of ' + str(unit) + ' unit in each of 3 layers with alpha = '+ str(alpha) +' is '+ 'train: ' + str(train) + ' test: ' + str(test)+"\n")
