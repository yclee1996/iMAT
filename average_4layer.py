...
def regress(unit,alpha):
    ...
    for i in range(0,5):
        product_model = MLPRegressor(hidden_layer_sizes=(unit,unit,unit,unit,),
                                     activation='relu',
                                     solver='lbfgs',
...
#print (regress())
unit_list = [100,150,200,250,300]
alpha_list = [0.0001,0.0003]
for alpha in alpha_list:
    for unit in unit_list:
        train, test = regress(unit,alpha)
        with open("NN_result_train_test_ave", "a") as p:
            p.write('the error of ' + str(unit) + ' unit in each of 4 layers with alpha = '+ str(alpha) +' is '+ 'train: ' + str(train) + ' test: ' + str(test)+"\n")
