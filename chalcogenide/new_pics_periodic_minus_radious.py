...
def get_molecule_dataset(molecule):
    path_list = get_certain_testset('/home/mx2/work/MChalc2/fix_spin',molecule)
    num = len(path_list)
    return_list = [None] * 4 *num
    for i in range(0,num):
        the_list= [None]
        the_list[0] = path_list[i]
        a,b = get_coord(the_list)
...
model = regress(300,0.0001,X_train,y_train)
