...
def perodic_table(molecule):
    ...
    for i in metal_atom:
        if i in molecule[-31:-26]:
            ...
            return_list.append(metal_atom.index(i)**2)
    for j in halo_atom:
        if j in molecule[-28:-25]:
            return_list.append(halo_atom.index(j))
            return_list.append(halo_atom.index(j)**2)
    return return_list
######################################
...
def get_coord(the_list):
    ...
    for c in the_list:
        try:
            with open(c, "r") as p:
                for i, line in enumerate(p):
                    ...
                    if "#" not in line:
                        ...
                        para_list.append(bond**2)
                        if len(perodic_table(c)) == 4:
            ...
            pass
    a = len(energy_list)
    parameter = np.array(para_list).reshape(a,9)
...
def draw_pic(a,molecule):
    ...
    for i in range(0,num):
        ax1.scatter(mole_dataset[4*i+3][:,0],mole_dataset[4*i+1], s=10, c=color_list[i], marker="o", label='real')
    error = [0] * len(mole_dataset[1].tolist())
    for i in range(0,num):
        for j in range(0,len(mole_dataset[1].tolist())):
            error[j] += abs(a.predict(mole_dataset[4*i])-mole_dataset[4*i+1]).tolist()[j]
    for x in range(0,len(error)):
        error[x] = error[x]/num * 100
    ax1.scatter(mole_dataset[3][:,0],error, s=10, c='black', marker="s", label='real')
    plt.show()
#####################################
def draw_all_pics(a):
    metal_atom = ['Sc','Ti','V','Cr','Mn','Fe','Co','Ni','Cu','Zn']
