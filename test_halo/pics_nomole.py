...
def perodic_table(molecule):
    return_list = []
    metal_atom = ['Sc','Ti','V','Cr','xxxx','xxxx','xxxx','xxxx','xxxx','xxxx']#'Mn','Fe','Co','Ni','Cu','Zn']
...
def draw_pic(a,molecule,train):
    ...
    for i in range(0,num):
        ...
        error =sum(abs(mole_dataset[4*i+1]-predict_list))/len(predict_list)
...
#        ax1.scatter(yes_list_o[:,0],a.predict(yes_list), s=120, c=color_list[i], marker="+", label='NN')
        ax1.scatter(other_list_o[:,0],a.predict(other_list)-the_min, s=90, c=color_list[i], marker="x", label='NN')
        ax1.set_xlabel('average error = '+str(error)[:5]+' eV')
    plt.show()
#####################################
...
for i in metal_atom:
    for j in halo_atom:
        all_mole_list.append(i+j)
for molecule in all_mole_list:
    all_vec_eng = get_all_file('/home/mx2/work/MH2/fix_spin',molecule)
