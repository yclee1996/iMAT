...
def perodic_table(molecule):
    ...
    metal_eatom = [1273.9758,1044.4404,1323.7742,1650.3862,2028.0466,597.3608,816.9435,1081.7953,0,0]
    halo_atom = ['F','Cl','Br','I']
    halo_row = [2,3,4,5]
    ...
    halo_eatom = [658.3685,410.3267,370.4479,318.2436]
    for i in metal_atom:
        if i in molecule[-23:-18]:
            ...
            return_list.append(metal_eatom[c])
    for j in halo_atom:
        if j in molecule[-20:-17]:
            ...
            return_list.append(halo_eatom[c])
    return return_list
######################################
...
def get_coord(the_list):
    ...
    for c in the_list:
        try:
            ...
            pass
    a = len(energy_list)
    parameter = np.array(para_list).reshape(a,17)
...
for alpha in alpha_list:
    for unit in unit_list:
        ...
        with open("NN_result_3layer_0.4", "a") as p:
            p.write('eatom_dis_radius_periodic_minus the error of ' + str(unit) + ' unit in each of 3 layers with alpha = '+ str(alpha) +' is '+ 'train: ' + str(train) + ' test: ' + str(test)+"\n")
