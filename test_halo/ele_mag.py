...
def regress(unit,alpha,X_train,y_train,X_test,y_test):
    ...
    return final_model
####################################
def get_predict_min(a,molecule):
    ...
    the_min = min_list[0]
    for i in range(0,num):
        mole_predict = a.predict(mole_dataset[4*i])-the_min
        predict_min = min(mole_predict)
        pre_min.append(predict_min)
    return pre_min
#####################################
def get_all_and_draw(a,mole_list):
    color_list = ['blue','green','red','black']
    metal_atom = ['Sc','Ti','V','Cr','Mn','Fe','Co','Ni','Cu','Zn']
    metal = mole_list[0][0:2]
    if metal != 'VF':
        fig = plt.figure(str(metal_atom.index(metal))+'_'+metal+'_predict',figsize=[1.2*len(get_predict_min(a,mole_list[0])),12])
        fig.suptitle(metal+'_predict')
    if metal == 'VF':
        fig = plt.figure('2_'+'V'+'_predict',figsize=[1.2*len(get_predict_min(a,mole_list[0])),12])
        fig.suptitle('2_'+'V'+'_predict')
    ax1 = fig.add_subplot(111)
    plt.xticks(np.arange(0, 5, step=1))
    plt.yticks(np.arange(-3, 3, step=1))
    for i in range(0,len(mole_list)):
        ax1.scatter(list(range(len(get_predict_min(a,mole_list[i])))),get_predict_min(a,mole_list[i]),s=60, c=color_list[i], marker="o", label='real')
        x = list(range(len(get_predict_min(a,mole_list[i]))))
        y = get_predict_min(a,mole_list[i])
...
get_all_and_draw(model,all_mole_list)
#for molecule in all_mole_list:
