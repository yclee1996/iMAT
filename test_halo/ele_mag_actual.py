            ...
            with open(c, "r") as p:
                ...
                for i, line in enumerate(p):
                    if "#" not in line:
                        ...
                        eng_each.append(f)
                        len_each.append(length)
            min_eng = min(eng_each)
            min_index = eng_each.index(min_eng)
            min_len = len_each[min_index]
            with open(c, "r") as p:
                for i, line in enumerate(p):
                    if "#" not in line:
                        length = float(line.split()[0])
                        ...
                        f = float(line.split()[1])
                        bond = 3
                        if ((min_len+0.4)>length) and (length>(min_len-0.4)):
...
def get_molecule_dataset(molecule):
    path_list = get_certain_testset('/home/mx2/work/MH2/fix_spin',molecule)
    num = len(path_list)
...
def get_predict_min(molecule):
    ...
    pre_min = []
    for i in range(0,num):
        predict_list = mole_dataset[4*i+1]
        the_min = min(predict_list)
        min_list.append(the_min)
    the_min = min_list[0]
    
    for i in range(0,num):
        mole_predict = (mole_dataset[4*i+1])-the_min
        predict_min = min(mole_predict)
        pre_min.append(predict_min)
    return pre_min
#####################################
def get_all_and_draw(mole_list):
    color_list = ['blue','green','red','black']
    metal_atom = ['Sc','Ti','V','Cr','Mn','Fe','Co','Ni','Cu','Zn']
    metal = mole_list[0][0:2]
    if metal != 'VF':
        fig = plt.figure(str(metal_atom.index(metal))+'_'+metal+'_actual',figsize=[1.2*len(get_predict_min(mole_list[0])),12])
        fig.suptitle(metal+'_actual')
    if metal == 'VF':
        fig = plt.figure('2_'+'V'+'_actual',figsize=[1.2*len(get_predict_min(mole_list[0])),12])
        fig.suptitle('2_'+'V'+'_actual')
    ax1 = fig.add_subplot(111)
    plt.xticks(np.arange(0, 5, step=1))
    plt.yticks(np.arange(-3, 3, step=1))
    for i in range(0,len(mole_list)):
        ax1.scatter(list(range(len(get_predict_min(mole_list[i])))),get_predict_min(mole_list[i]),s=60, c=color_list[i], marker="o", label='real')
        x = list(range(len(get_predict_min(mole_list[i]))))
        y = get_predict_min(mole_list[i])
        plt.plot(x, y, linewidth=3.0,c=color_list[i])
    ax1.scatter([0,0],[-3,2],s=3,c='white',marker="o")
    plt.show()
#####################################
...
#        predict_list = []
#  
#        for c in range(0,len(mole_predict)):
#            if mole_predict[c] < 1.5:
#                inrange_list += other_list_o[c].tolist()
#                predict_list.append(mole_predict[c])
#        inrange_list = np.array(inrange_list).reshape(len(predict_list),13)
#        ax1.scatter(inrange_list[:,0],predict_list, s=90, c=color_list[i], marker="x", label='NN')
...
all_vec_eng = get_all_file('/home/mx2/work/MH2/fix_spin','zzzzz')
...
model = regress(5,0.001,X_train,y_train,X_test,y_test)
halo_atom = ['F2','Cl2','Br2','I2']

for atom in metal_atom:
    all_mole_list = []
    for j in halo_atom:
        all_mole_list.append(atom+j)
    get_all_and_draw(all_mole_list)
#for molecule in all_mole_list:
