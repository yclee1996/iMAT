            ...
            with open(c, "r") as p:
                for i, line in enumerate(p):
                    if "#" not in line:
                        ...
                        if ((min_len+0.2)>length) and (length>(min_len-0.2)):
...
def draw_pic(a,molecule,train):
    ...
    fig = plt.figure(molecule+'0.2_0.8',figsize=[12,9])
    fig.suptitle(molecule+'0.2_0.8')
...
X_train, X_test, y_train, y_test = train_test_split(scaled_para, y, random_state = 3, test_size=0.2)
